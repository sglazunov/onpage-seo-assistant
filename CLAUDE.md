...
- **React переиспользует DOM-узлы между маршрутами.** При смене маршрута `<h1>` остаётся
  тем же узлом, меняется только текст. Поэтому подсветка снимается не по `isConnected`, а
  по снимку содержимого (`contentSnapshot`), который сверяет `MutationObserver`.
- **Позиционный селектор переживает перерисовку и указывает на чужой элемент.**
  `li:nth-of-type(2) > a` после смены маршрута резолвится в другую ссылку. Поэтому
  `highlight`/`scrollTo` принимают `verify` — текст или `src` из отчёта — и отказываются
  подсвечивать несовпадающий элемент.
